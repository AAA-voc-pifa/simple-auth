...
import { parse_json } from '#/request-util.ts'

async function read_body(req: Request) {
	const { email, code } = await parse_json(req) as { email: unknown, code: unknown }
	if (is_str(email) && is_email(email) && is_str(code) && code.length === 6)
		return { email, code }
